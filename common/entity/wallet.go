...
type WalletAddressGenerateReq struct {
	...
	AddressName string         `json:"address_name" binding:"required" example:"deposit-address"`       // 地址名称，如："用户充值"
}
...
type WalletAddressGenerateRespData struct {
	...
	CreatedTimestamp int64  `json:"created_timestamp"` // 创建时间戳（毫秒）
}
...
type PageParamReq struct {
	...
	StartId string `json:"start_id" example:"6a0d6d6ad91627d0d1c763cd"` // 可选，定义当前列表应从上一个已列出的记录的 ID 开始
}
type PageParamResp struct {
	Limit   int    `json:"limit"`    // 返回记录数限制
	StartId string `json:"start_id"` // 下一页应传的游标 ID（本页最后一条记录的 ID），无更多数据时为空
...
type WalletItem struct {
	...
	CreatedTimestamp int64         `json:"created_timestamp"` // 钱包创建时间戳（毫秒）
	IsMaster         bool          `json:"is_master"`         // 是否为主钱包
...
type AssetItem struct {
	...
	AssetType       AssetType       `json:"asset_type"`         // 资产类型，原生币（coin）或代币（token）
	Contract        string          `json:"contract,omitempty"` // 代币合约地址，仅对代币（token）有效
	AvailableAmount decimal.Decimal `json:"available_amount"`   // 可用余额
	AllocatedAmount decimal.Decimal `json:"allocated_amount"`   // 待处理余额
}
